...
function Diagram(callbacks, areaEl, arrowButtonEl, contextEl, arrowReferenceEl) {
    ...
    callbacks.initArrows(arrow => arrow.setDiagram(this));
    ...
    function addArrow() {
        if (drawingArrow) {
            ...
            if (startArrow) {
                drawingArrow = new Arrow(_this, drawingArrow.endDevice);
                startArrow = false;
            } else {
                drawingArrow = null;
                deactivateArrowDrawing();
        ...
        if (!ui.helper.hasClass("device-in-area")) {
            ...
            if (pos[0] >= 0 && pos[0] + width <= _this.area.width()
                ...
                const source = ui.draggable,
                    ...
                    updateFunction = update[data.type];

                data.control.log = [];
                data.control.current = data.control.min || 0;
                const device = new Device(_this, index, pos, data.type, title, data.control, image, updateFunction);
    ...
    function deviceMouseDown(device) {
        ...
        if (selectedDevice === device) {
            ...
            if (startArrow && !drawingArrow) {
                ...
                drawingArrow = new Arrow(_this, device);
                startArrow = false;
